    ...
    for col in df.columns:
        ...
        if col_type in ("Int32", "Int64", "Float32", "Float64"):
            ...
            if len(non_nulls) > 0:
                ...
                is_binary = (
                    col_schema.get("semantic_type") == "binary"
                ...
                if is_binary and len(unique_vals) == 2:
                    ...
                    if min_ratio < CLASS_IMBALANCE_MIN:
                        ...
                        if not any(iss["column"] == col and iss["type"] == "CLASS_IMBALANCE" for iss in profile["critical_issues"]):
                            profile["critical_issues"].append({
                                "column": col,
...
def _check_class_imbalance(
    df: pl.DataFrame, target_col: str, profile: Dict[str, Any]
) -> None:
