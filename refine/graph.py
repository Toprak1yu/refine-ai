...
def profile_node(state: AgentState) -> Dict[str, Any]:
    """Loads raw data, transforms to Polars, and computes statistical profile."""
    ...
    return {
        ...
        "audit_trail": state.get("audit_trail", []) + ["Initial dataset profiling completed."]
    }
...
def deterministic_clean_node(state: AgentState) -> Dict[str, Any]:
    ...
    df, logs = run_deterministic_clean(df)
...
def evaluate_anomalies_node(state: AgentState) -> Dict[str, Any]:
    ...
    if critical_issues and not state.get("human_resolutions"):
        ...
        human_decisions = interrupt({
            ...
            "available_strategies": ["DROP", "STATISTICAL_IMPUTE", "SYNTHETIC_SYNTHESIS"]
        })
...
def apply_resolutions_node(state: AgentState) -> Dict[str, Any]:
    ...
    if resolutions:
        df, logs = apply_human_resolutions(df, resolutions)
        return {
            "records": df.to_dicts(),
            "audit_trail": state["audit_trail"] + logs
        }
...
def export_node(state: AgentState) -> Dict[str, Any]:
    ...
    df = pl.DataFrame(state["records"])
    output_path = state.get("processed_file_path") or "data/processed/clean_customers.csv"
